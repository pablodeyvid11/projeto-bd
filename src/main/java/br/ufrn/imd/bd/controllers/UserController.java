...
public class UserController {
	...
	public ResponseEntity<?> signon(@RequestBody(required = true) @Valid UserDTO signonRecord,
		...
		try {
			...
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body(new ApiErrorDTO(HttpStatus.INTERNAL_SERVER_ERROR.value(), "/user", "Error on register user"));
		}
	}
	...
	public ResponseEntity<?> signin(@RequestBody(required = true) @Valid SigninDTO signonRecord,
		...
		try {
			...
			return ResponseEntity.status(HttpStatus.BAD_REQUEST)
					.body(new ApiErrorDTO(HttpStatus.BAD_REQUEST.value(), "/user/login", "Login incorreto"));
		}
	}
		...
		try {
			...
			if (!u.getEmail().equals(signonRecord.email())) {
				return ResponseEntity.badRequest()
						.body(new ApiErrorDTO(HttpStatus.BAD_REQUEST.value(), "/user", "You can't edit others user"));
			}
			...
			return ResponseEntity.status(HttpStatus.BAD_REQUEST)
					.body(new ApiErrorDTO(HttpStatus.BAD_REQUEST.value(), "/user", e.getMessage()));
		} catch (Exception e) {
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body(new ApiErrorDTO(HttpStatus.INTERNAL_SERVER_ERROR.value(), "/user", "Error on update user"));
		}
	}
	...
	public ResponseEntity<?> delete(@PathVariable(name = "id") Long id,
		...
		try {
			...
			return ResponseEntity.status(HttpStatus.BAD_REQUEST)
					.body(new ApiErrorDTO(HttpStatus.BAD_REQUEST.value(), "/user/" + id, e.getMessage()));
		} catch (Exception e) {
