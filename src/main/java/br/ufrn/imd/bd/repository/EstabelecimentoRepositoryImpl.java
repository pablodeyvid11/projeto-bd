...
public class EstabelecimentoRepositoryImpl implements EstabelecimentoRepository {
	...
	@Override
	public Estabelecimento findById(Long cnpj) {
	...
	public void save(Estabelecimento estabelecimento) {
		String sql = String.format(
				"INSERT INTO %s (cnpj, nome, endereco_bairro, endereco_rua, endereco_cep, endereco_numero) VALUES (?, ?, ?, ?, ?, ?)",
				getTableName());
		try (Connection connection = dataSource.getConnection();
				PreparedStatement ps = connection.prepareStatement(sql)) {
			ps.setLong(1, Long.parseLong(estabelecimento.getCnpj()));
	...
	public void update(Estabelecimento estabelecimento) {
		String sql = String.format(
				"UPDATE %s SET nome = ?, endereco_bairro = ?, endereco_rua = ?, endereco_cep = ?, endereco_numero = ? WHERE cnpj = ?",
				getTableName());
		try (Connection connection = dataSource.getConnection();
				PreparedStatement ps = connection.prepareStatement(sql)) {
			...
			ps.setLong(6, Long.parseLong(estabelecimento.getCnpj()));
			ps.executeUpdate();
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	private Estabelecimento mapRowToEstabelecimento(ResultSet rs) throws SQLException {
		...
		estabelecimento.setCnpj(rs.getLong("cnpj") + "");
